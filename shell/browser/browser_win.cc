...
int Browser::GetBadgeCount(base::Optional<bool> shouldGetBadge) {
  if (!shouldGetBadge.has_value() || !shouldGetBadge.value()) {
    return badge_count_;
  } else {
    return 0;
  }
...
bool Browser::SetBadgeCount(base::Optional<int> count,
  ...
  if (!shouldSetBadge.has_value() || !shouldSetBadge.value()) {
    // app.setBadgeCount(count) is not support on Windows, but
    // navigator.setAppBadge is
    return false;
  }
  ...
  for (auto* window : WindowList::GetWindows()) {
    ...
    // On Windows set the badge on the first window found for this process.
    if (process_id == GetCurrentProcessId()) {
      UpdateBadgeContents(hwnd, badge_content, badge_alt_string);
    }
  }
  return true;
}
